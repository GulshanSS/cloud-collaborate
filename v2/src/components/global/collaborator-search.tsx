...
import { getUserFormSearch } from "@/lib/supabase/queries";

interface CollaboratorSearchProps {
  existingCollaborators: User[] | [];
  getCollaborators: (collaborators: User) => void;
...
const CollaboratorSearch: React.FC<CollaboratorSearchProps> = ({
  ...
  const onChangeHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    timerRef.current = setTimeout(async () => {
      const res = await getUserFormSearch(e.target.value);
    ...
    <Sheet>
      ...
      <SheetContent className="w-[400px] sm:w-[540px]">
        ...
        <div className="flex justify-center items-center">
          <Search />
          <Input name="name" placeholder="Email" onChange={onChangeHandler} />
        </div>
