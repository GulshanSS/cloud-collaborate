...
const QuillEditor: React.FC<QuillEditorProps> = ({
  ...
  const onChangeIcon = async (icon: string) => {
    if (!fileId) return;
    if (dirType === "workspace") {
      await updateWorkspace(fileId, { iconId: icon });
      dispatch({
    ...
    if (dirType === "folder") {
      if (!workspaceId) return;
      await updateFolder(fileId, { iconId: icon });
      dispatch({
        type: "UPDATE_FOLDER",
        payload: {
          workspaceId,
    ...
    if (dirType === "file") {
      ...
      await updateFile(fileId, { iconId: icon });
      dispatch({
        type: "UPDATE_FILE",
        payload: {
          workspaceId,
  ...
  const deleteBanner = async () => {
    ...
    if (dirType === "file") {
      ...
      await updateFile(fileId, { bannerUrl: "" });
    }
    ...
    if (dirType === "folder") {
      ...
      await supabase.storage.from("file-banners").remove([`banner.${fileId}`]);
      await updateFolder(fileId, { bannerUrl: "" });
    }
    ...
    if (dirType === "workspace") {
      ...
      await supabase.storage.from("file-banners").remove([`banner.${fileId}`]);
      await updateWorkspace(fileId, { bannerUrl: "" });
    }
  };
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:justify-between justify-center sm:items-center sm:p-2 p-8">
          ...
          <div className="flex items-center gap-4">
                ...
                <TooltipProvider key={collaborator.id}>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Avatar
                        className="-ml-3 bg-background
                    border-2 flex items-center justify-center border-white h-8 w-8 rounded-full"
                      >
                        <AvatarImage
                          src={
            ...
            {saving ? (
              <Badge
                variant={"secondary"}
                className="bg-orange-600 text-white top-4 right-4 z-50"
              >
                Saving...
              </Badge>
            ) : (
              <Badge
                variant={"secondary"}
                className="bg-green-600 text-white top-4 right-4 z-50"
              >
                Saved
              </Badge>
            )}
        ...
        <div className="w-full self-center max-w-[800px] flex flex-col px-7 lg:my-8">
          ...
          <div className="flex">
            <BannerUpload
              id={fileId}
              dirType={dirType}
              className="mt-2 text-sm text-muted-forground p-2 hover:text-card-foreground transition-all rounded-md"
            >
